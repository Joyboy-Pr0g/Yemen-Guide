...
const emptyValues = (): Omit<SiteSettings, 'logo'> => ({
  site_name: '',
  site_description: '',
  meta_keywords: '',
  contact_email: '',
...
export default function AdminSettingsContent({ initialSettings }: { initialSettings: SiteSettings }) {
  const { data: settings } = useAdminSettings(initialSettings)
  const { mutate: updateSettings, isPending } = useUpdateSettings()
  const { mutate: uploadLogo, isPending: uploadingLogo } = useUploadLogo()
  const [values, setValues] = useState<Omit<SiteSettings, 'logo'>>(emptyValues())
